...
byte xram bKey1, bKey2, bTmpX, bTmpY, bBrk, bTailX, bTailY;
...
RND_SKIP:		
		...
		if (bLoopNr == 10) { // delay
			...
			// clear tail of snake (oldest position in circular buffer)
			bTailX = bXSnake[bSnakePos];
			bTailY = bYSnake[bSnakePos];
			lcd_pset(bTailX, bTailY, 0);

			// boundary check and direction update
			if (bTmpX == 0) bDxBall=1;
			if (bTmpX == 99) bDxBall=0;
			if (bTmpY == 1) {
				bDyBall=1;
			...
			// store new head in circular buffer, advance tail pointer
			bXSnake[bSnakePos] = bTmpX;
			bYSnake[bSnakePos] = bTmpY;

			bLoopNr = 0;
			bSnakePos++;
			if (bSnakePos == 4) bSnakePos = 0;
		}
