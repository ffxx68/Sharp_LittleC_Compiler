...
-- Emulate a key press on the specified input port and mask
function key_press(input_tag, input_mask)
	...
	if input_port then
		for k,field in pairs ( input_port.fields ) do
			...
			-- Emulate key press
				field:set_value(1)
				emu.wait(0.05)  -- Wait for the specified duration
				-- Emulate key release
				field:set_value(0)
				break
			end
...
-- send a line of keystrokes + ENTER to the emulator
function keyline (input_str)
-- echo
	print(input_str)
...
-- invoke 'keyline' for each line in the input file
...
-- send a single key to the emulator
function key(char)
	...
	switch (char) {
		...
		-- Special buttons mapped to reserved characters
		--	 sml   ":KEY1"  0x80
		--   shift ":KEY1"  0x20
		--   basic ":KEY11" 0x80
		["|"] = function()  key_press (":KEY11", 0x80) end ,
		--   cal   ":KEY12" 0x80
		["_"] = function()  key_press (":KEY12", 0x80) end ,
		--   c-ce  ":KEY7"  0x02

		-- Still missing : the scientific functions keys (hyp, sin, ...)
		-- . . .
